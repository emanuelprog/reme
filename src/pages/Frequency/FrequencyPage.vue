    ...
    <q-page class="q-pa-md" :class="$q.dark.isActive ? 'bg-grey-10' : 'bg-white'">
        <div class="column items-center">
            ...
            <template v-else>
                <template v-if="dataLoaded && students.length">
                    <q-pagination v-if="filteredDateColumns.length > columnsPerPage" v-model="columnPage"
                        :max="Math.ceil(filteredDateColumns.length / columnsPerPage)" color="primary" input
                        boundary-numbers class="q-mb-md" />
                            ...
                            <q-tr :props="props" :class="props.row.hasOccurrence ? 'bg-red-2 text-red-10' : ''">
                                <q-td v-for="col in props.cols" :key="col.name" :props="props"
                                    ...
                                    <template v-if="paginatedDateColumns.includes(col.name)">
                                        <q-select v-model="props.row.frequencies[col.name]" :options="attendanceOptions"
                                            filled dense emit-value map-options class="frequency-cell" :disable="!(
                                    ...
                                    <template v-else-if="col.name === 'observation'">
                                        <q-btn v-if="props.row.hasObservation" flat round dense icon="chat_bubble"
                                            color="primary" @click="() => openObservationModal(props.row.id)" />
                                    </template>
            ...
            <div class="row q-gutter-md justify-center q-my-lg">
                <q-btn label="Salvar Frequências" color="primary" icon="save" :disable="!dataLoaded || loading"
                    @click="onSaveFrequencies" />
                <q-btn label="Enviar Frequências" color="warning" icon="send" :disable="!dataLoaded || loading"
